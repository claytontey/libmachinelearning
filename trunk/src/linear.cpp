...
struct model *load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		{
			result = fscanf(fp,"%d",&nr_class);
			model_->nr_class=nr_class;
		}
		else if(strcmp(cmd,"nr_feature")==0)
		{
			result = fscanf(fp,"%d",&nr_feature);
			model_->nr_feature=nr_feature;
		}
		else if(strcmp(cmd,"bias")==0)
		{
			result = fscanf(fp,"%lf",&bias);
			model_->bias=bias;
		}
		...
		else if(strcmp(cmd,"label")==0)
		{
			...
			for(int i=0;i<nr_class;i++)
				result = fscanf(fp,"%d",&model_->label[i]);
		}
		else
		{
	...
	for(i=0; i<n; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
			result = fscanf(fp, "%lf ", &model_->w[i*nr_w+j]);
		result = fscanf(fp, "\n");
	}
