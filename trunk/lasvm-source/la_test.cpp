...
void test(char *output_name)
{
    ...
    int i,j, tmp; double y; double acc=0;
...
int main(int argc, char **argv)
{
    ...
    gettimeofday(&tic,NULL);  test(output_file_name); gettimeofday(&toc,NULL);
    testtime = ((toc.tv_sec-tic.tv_sec)*1000.0 + (toc.tv_usec-tic.tv_usec)*0.001)/1000.0;
   
    sprintf(testtimefilename,"%s.time",input_file_name);
